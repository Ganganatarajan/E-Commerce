...
import ViewAdmin from './ViewAdmin'; // Modal Component
...
const Admins = () => {
  ...
  const [admins, setAdmins] = useState([
    { id: 1, Name: "Paradise JohnDoe", EmailID: "JohnDoe67@gmail.com", MobileNumber: "9876543210" },
    { id: 2, Name: "Sunshine Resid", EmailID: "sunshine567@gmail.com", MobileNumber: "8765432190" },
    { id: 3, Name: "Green Valley", EmailID: "greenvalley@gmail.com", MobileNumber: "9123456789" },
    { id: 4, Name: "Skyline Admin", EmailID: "skylineadmin@gmail.com", MobileNumber: "9998887776" },
    { id: 5, Name: "Ocean View", EmailID: "oceanview@gmail.com", MobileNumber: "9090909090" }
  ]);

  useEffect(() => {
    document.body.style.overflow = selectedAdmin ? 'hidden' : '';
  }, [selectedAdmin]);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentData.length > 0 ? currentData.map((item, index) => (
                ...
                <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-500">No data found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {filteredData.length > 0 && totalPages > 1 && (
        <div className="flex justify-end mt-4 items-center space-x-2">
          <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
            className={`px-3 py-1 rounded-lg border text-sm font-medium ${currentPage === 1 ? "bg-gray-200 text-gray-400" : "hover:bg-purple-100"}`}
            disabled={currentPage === 1}>Prev</button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
            <button key={page} onClick={() => setCurrentPage(page)}
              className={`px-3 py-1 rounded-lg text-sm font-medium border ${page === currentPage ? "bg-indigo-600 text-white" : "hover:bg-purple-100"}`}>
              {page}
            </button>
          ))}
          <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))}
            className={`px-3 py-1 rounded-lg border text-sm font-medium ${currentPage === totalPages ? "bg-gray-200 text-gray-400" : "hover:bg-purple-100"}`}
