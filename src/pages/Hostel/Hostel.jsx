...
import { GetHostel, deleteHostel as deleteHostelAPI } from "../../Services/Hostel";
...
const Hostel = () => {
  ...
  const filteredData = hostels.filter((item) =>
    [item.hostelName, item.contactPerson, item.mobileNumber, item.city, item.area].some((field) =>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {["S.No", "Hostel Name", "Contact Person", "Mobile", "City", "Area", "Verified", "Actions"].map((title, i) => (
              ...
              {currentData.length > 0 ? (
                currentData.map((item, index) => (
                  <tr key={item._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{item.hostelName}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{item.contactPerson}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{item.area}</td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() => toggleVerified(item._id)}
                        className={`px-3 py-1 text-xs font-semibold rounded-full transition duration-300 ${
                          item.verified ? "bg-green-500 text-white hover:bg-green-600" : "bg-red-500 text-white hover:bg-red-600"
                        }`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        ...
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2h1v10a2 2 0 002 2h6a2 2 0 002-2V6h1a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM8 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" clipRule="evenodd" />
                          </svg>
                        </button>
                      </div>
                    </td>
